...
class Post {
    ...
    func adjustLikes(addLike: Bool, completion: @escaping (Int) -> Void) {
        ...
        if addLike {
            ...
            USER_LIKES_REF.child(currentUid).child(postID).observeSingleEvent(of: .value) { snapshot in
                ...
                // 서버에서 노티 제거
                NOTIFICATIONS_REF.child(ownerUID).child(notificationID).removeValue { err, ref in
    ...
    func deletePost() {
        ...
        POST_LIKES_REF.child(postID).observe(.childAdded) { snapshot in
            ...
            // 유저가 좋아한 게시물
            USER_LIKES_REF.child(uid).child(postID).observeSingleEvent(of: .value) { snapshot in
                guard let notificationId = snapshot.value as? String else { return }
                guard let ownerUID = self.ownerUID else { return }
    ...
    func sendLikeNotificationToServer() {
        guard
            let currentUid = Auth.auth().currentUser?.uid,
            let postID = postID,
            let ownerUID = ownerUID else { return }
