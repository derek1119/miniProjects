...
class EntryViewController: UIViewController, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        //navigationItem 중 rightbarButton에 save 버튼을 만들어서 done이라는 어떤 이벤트와 이전화면으로 돌아가게 하는 스타일로 표현했다. target은 액션 메세지를 받는 객체인데 self라는 것은 (아마도?)버튼 자체에서 액션 메세지를 받는다는 의미인 것 같다. #selector란 함수를 직접 지정하는 기능을 가진 일종의 함수 선택자이다. 본래 object-c에서 클래스 메소드의 이름을 가르키는데 사용되는 참조타입이다.
    }
